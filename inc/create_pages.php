...
if (isset($_GET['activated']) && is_admin()){
	$blog_page_title = _x('Blog', 'the-towing-theme');
...
if (isset($_GET['activated']) && is_admin()){
	$contact_page_title = _x('Contact', 'the-towing-theme');
...
if (isset($_GET['activated']) && is_admin()){
	$sitemap_page_title = _x('Site Map', 'the-towing-theme');
...
if (isset($_GET['activated']) && is_admin()){
	$services_page_title = _x('Our Services', 'the-towing-theme');
	$services_page_check = get_page_by_title($services_page_title);
	$services_page = array(
		'post_type' => 'page',
		'post_title' => $services_page_title,
		'post_status' => 'publish',
		'post_author' => 1,
		'post_slug' => 'site-map'
	);
	if(!isset($services_page_check->ID) && !the_slug_exists('our-services')){
		$services_page_id = wp_insert_post($services_page);
	}
}
// create the about us page
if (isset($_GET['activated']) && is_admin()){
    $about_page_title = _x('About Us', 'the-towing-theme');
...
if (isset($_GET['activated']) && is_admin()){
	$home_page_title = _x('Home', 'the-towing-theme');
	$home_page_content = '';
